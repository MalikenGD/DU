...
namespace InfiniteVoid.SpamFramework.Core.Common
{
    ...
    public class AbilityEffectsApplier
    {
        ...
        private const string LOG_MODULE = SpamLogModules.EFFECTS_APPLIER;
        ...
        {
            SpamLogger.LogDebug(LOG_MODULE, "Start applying effects");
            _invoker = invoker;
            _hitpos = hitPosition;
        ...
        public void Stop()
        {
            this._curState = ApplyEffectsState.Done;
        }
        ...
        private ApplyEffectsState ApplyEffectToTarget()
        {
            ...
            {
                SpamLogger.LogDebug(LOG_MODULE, $"Apply {_curEffect.Effect.Name} to {_invoker.Transform.name}");
                _curEffect.Effect.ApplyTo(_invoker, _hitpos, _ability, _invoker);
            }
            else
            {
                SpamLogger.LogDebug(LOG_MODULE, $"Apply {_curEffect.Effect.Name} to {_curTarget.Transform.name}");
                _curEffect.Effect.ApplyTo(_curTarget, _hitpos, _ability, _invoker);
            }

            if (_ability.ApplyPerTarget)
            {
                SpamLogger.LogDebug(LOG_MODULE, $"Apply per target. Set state to {ApplyEffectsState.WaitForNextEffect}");
                return ApplyEffectsState.WaitForNextEffect;
            }
            else
            {
                SpamLogger.LogDebug(LOG_MODULE, $"Apply per effect. Set state to {ApplyEffectsState.WaitForNextEffect}");
                return ApplyEffectsState.WaitForNextTarget;
            }
        }
        ...
        private ApplyEffectsState NoMoreEffects()
        {
            ...
            if (!_ability.ApplyPerTarget)
            {
                SpamLogger.LogDebug(LOG_MODULE, "Apply effects done. No more effects and not applying per target.");
            ...
            SpamLogger.LogDebug(LOG_MODULE, $"Set state to {ApplyEffectsState.WaitForNextTarget}");
            return ApplyEffectsState.WaitForNextTarget;
        }
        ...
        private ApplyEffectsState NoMoreTargets()
        {
            ...
            if (_ability.ApplyPerTarget)
            {
                SpamLogger.LogDebug(LOG_MODULE, "Apply effects done. No more targets and effects applied by target.");
                return ApplyEffectsState.Done;
            }
        ...
        private ApplyEffectsState GetNextEffect()
        {
            ...
            {
                SpamLogger.LogDebug(LOG_MODULE, $"Set state to {ApplyEffectsState.NoMoreEffects}");
            ...
            {
                SpamLogger.LogDebug(LOG_MODULE, $"Set state to {ApplyEffectsState.ApplyEffectToTarget}");
        ...
        private ApplyEffectsState GetNextTarget()
        {
            ...
            {
                SpamLogger.LogDebug(LOG_MODULE, $"Target was the last. Set state to {ApplyEffectsState.NoMoreTargets}");
            ...
            if (_curTarget == null)
            {
                SpamLogger.LogDebug(LOG_MODULE, $"Target was last or null. Set state to {ApplyEffectsState.NoMoreTargets}");
                return ApplyEffectsState.NoMoreTargets;
            }
        ...
        private void ApplyConditionalEffects()
        {
            ...
            if(_curTarget.Transform)
                SpamLogger.LogDebug(LOG_MODULE,$"Apply conditional effects to {_curTarget.Transform.name}");
            #endif
