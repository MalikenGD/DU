...
namespace InfiniteVoid.SpamFramework.Core.Effects
{
    ...
    public class TeleportEffectSO : AbilityEffectSO
    {
            ...
            AbilityInvoker invoker)
        {
            ...
            {
                SpamLogger.LogDebug(_logModule, $"Teleport {invoker.Transform.name} to {target.Transform.name}");
            ...
            {
                SpamLogger.LogDebug(_logModule, $"Teleport {target.Transform.name} to {invoker.Transform.name}");
