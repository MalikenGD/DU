...
#if UNITY_EDITOR
            if (_logDamage)
                SpamLogger.EditorOnlyLog(invoker, () => $"{invoker.transform.name} deals {_damageValue} damage to {target.Transform.name}");
#endif
            SpamLogger.LogDebug(invoker, LOG_MODULE, () => $"{invoker.transform.name} deals {_damageValue} damage to {target.Transform.name}");
